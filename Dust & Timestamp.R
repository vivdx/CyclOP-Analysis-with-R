...
dust<-matrix(dustdata)
dust
mid<-matrix(midata) 
mid

table<-data.frame(dust)
table

#mean of dust table (all values)
x<-as.numeric(c(dust))
x
mean<-mean(x)
...
dbExistsTable(con,"analysis_dust")

dbSendQuery(con, paste("insert into analysis_dust (standard_deviation,time,variance,mean,id) VALUES ( '",sd,"','",time,"','",v,"','",mean,"','",mid,"');",sep=""))
